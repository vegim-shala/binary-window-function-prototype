...
void JoinUtils2::pretty_print_segment_tree() const {
    if (segtree.empty()) {
...
void JoinUtils2::build_index(const Dataset& input, const FileSchema &schema, std::string& value_column) {
...
void JoinUtils2::build_index_from_vectors_segtree(const std::vector<uint32_t> &sorted_keys, const std::vector<uint32_t> &values) {
    n = sorted_keys.size();
...
double JoinUtils2::seg_query(size_t l, size_t r) const {
    double res = 0.0;
...
void JoinUtils2::build_index_from_vectors_segtree_top_down(const std::vector<uint32_t> &sorted_keys, const std::vector<uint32_t> &values) {
    n = sorted_keys.size();
    keys = sorted_keys; // copy
...
double JoinUtils2::seg_query_top_down(size_t ql, size_t qr) const {
...
void JoinUtils2::build_index_from_vectors_prefix_sums(const std::vector<uint32_t> &sorted_keys,
...
double JoinUtils2::prefix_sums_query(size_t l, size_t r) const {
    // r is exclusive
    return prefix_sums[r] - prefix_sums[l];
}

void JoinUtils2::build_index_from_vectors_sqrt_tree(const std::vector<uint32_t> &sorted_keys,
...
double JoinUtils2::sqrt_query(size_t l, size_t r) const {
...
void JoinUtils2::build_index_from_vectors_two_pointer_sweep(const std::vector<uint32_t> &sorted_keys,
                                         const std::vector<uint32_t> &vals) {
    n = sorted_keys.size();
    keys = sorted_keys;
    values = vals;
...
// std::vector<uint32_t> JoinUtils2::sweep_query(
...
double JoinUtils2::compute_sum_range(const Dataset& input, const FileSchema &schema, const DataRow& probe_row,
...
std::vector<size_t> JoinUtils2::compute_range_join(
...
void JoinUtils2::validate() const {
...
std::vector<size_t> JoinUtils2::compute_join(const Dataset& input, const FileSchema &schema, const DataRow& probe_row) const {
    validate();
    if (join_spec.type == JoinType2::RANGE) {
        return compute_range_join(input, schema, probe_row, join_spec.begin_column, join_spec.end_column);
    }
    // else if (join_spec.type == JoinType2::RANGE) {
